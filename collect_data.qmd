...
1. **Weekly Drought Monitor records** for Oglala Lakota County, South Dakota (FIPS 46102).
...
# --- Task 1: Fetch U.S. Drought Monitor Data for Oglala Lakota County (FIPS 46102) ---
fips_code = "46102"
usdm_url = (
...
## U.S. Drought Monitor Data Summary
...
print(f"Number of weekly USDM records: {len(df_usdm)}")
df_usdm[["ValidStart", "County", "None", "D0", "D1", "D2", "D3", "D4"]].tail()
```

### Drought Summary Stats
...
fig, axes = plt.subplots(2, 1, figsize=(7.5, 6.5), sharex=False)

# 1. Plot Weekly Drought Monitor Trends (Cumulative Area Percentage)
...
axes[0].set_title("Weekly Drought Monitor Area Severity (%) - Oglala Lakota County", fontsize=13, fontweight="bold")
axes[0].set_ylabel("Cumulative % Area affected", fontsize=11)
axes[0].set_ylim(0, 100)
axes[0].legend(loc="upper right", frameon=True)

# 2. Plot Monthly Aggregated Precipitation (for cleaner visualization)
...
axes[1].bar(df_monthly["date_index"], df_monthly["precipitation_inches"], width=25, color="#1f77b4", alpha=0.8)
# Plot a 12-month rolling mean to show trends
df_monthly["rolling_mean"] = df_monthly["precipitation_inches"].rolling(12, center=True).mean()
axes[1].plot(df_monthly["date_index"], df_monthly["rolling_mean"], color="#d62728", linewidth=2.5, label="12-Month Rolling Mean")

axes[1].set_title("South Dakota Monthly Total Precipitation (Statewide Average)", fontsize=13, fontweight="bold")
axes[1].set_ylabel("Precipitation (inches)", fontsize=11)
