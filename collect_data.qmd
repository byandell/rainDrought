...
county_bullets = "".join([f"{c['name']} (FIPS {c['fips']}), " for c in config["counties"]])
...
1. **Weekly Drought Monitor records** for {county_bullets} {config['state_name']}.
2. **Daily Precipitation records** for the state of {config['state_name']} (state-wide mean).

The default datasets are saved to the [`data`](https://github.com/byandell/rainDrought/tree/main/data) directory.
